...
$sql_my_publications = $con->prepare("SELECT p.id_publicacion, u.primer_nombre AS primer_nombre, u.primer_apellido AS primer_apellido, p.titulo, p.descripcion, p.imagen, p.fecha, p.hora, p.visualizaciones, CASE WHEN nlg.id_usuario IS NOT NULL THEN 1 ELSE 0 END AS no_le_gusta FROM publicacion p JOIN usuario u ON u.id = p.id_autor LEFT JOIN no_le_gusta nlg ON p.id_publicacion = nlg.id_pub AND nlg.id_usuario = ? WHERE p.id_autor != ? AND oferta != 1 ORDER BY no_le_gusta ASC, p.id_publicacion DESC;");
